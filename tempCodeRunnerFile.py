...
pygame.init() #we will initialize the pygame library.

width = 700 #The width and height variable will be used to set the size of the display screen
height = 600

display = pygame.display.set_mode((width, height))#display basically stores the pygame screen that is shown using the set_mode() function.
pygame.display.set_caption(" Balloon Shooter Game")#sets the name of the screen to "" Balloon Shooter Game”.
clock = pygame.time.Clock()#Keeps track of time.

margin = 100 #Stores the margin value
lowerBound = 100 
score = 0 #Keeps track of the score. This value is updated as the player bust the balloons.
...
font = pygame.font.SysFont("Arial", 25) #takes in the name of the font and the size of the font.

class Balloon:
    def __init__(self, speed):#_ function to assign the value for speed
    ...
    def move(self):
        ...
        if self.y + self.b < 0 or self.y > height + 30:
            self.reset()
         #    displaying the balloons on the screen.
    def show(self):
...
def close():
    pygame.quit()
    sys.exit()
    #event.type() to compare the events
def game():
    global score
    loop = True
    ...
    while loop:
        ...
        clock.tick(60)
       # simply a call to the game() function in order to start the game
game()
